...
Now we need to execute that program in jBASE. 
  
1. Now go back to Jbase (command prompt window).
1. and to edit the program type in `JED BP MAKE.DEMO`. 
...
Now we are ready to create the demo file by running the program with the command `RUN BP MAKE.DEMO`. The program will ask two questions first the number of records you want to create and the file name please use *DEMO.FILE* for the filename as that is what will be used in future videos.  The program randomly generates the data for the a static list of each data element. Lastly we list the file to see that the file was created and populated the command is `LIST DEMO.FILE FIRSTNAME LASTNAME ADDR1`.
